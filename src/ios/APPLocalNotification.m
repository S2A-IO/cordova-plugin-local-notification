 ...
 *
 *  ▸ Remote push ➜ forward to the previous delegate (e.g. FCM) once.
 *  ▸ Local        ➜ handle inside this plugin, exactly as before.
 */
         ...
         withCompletionHandler:(void (^)(UNNotificationPresentationOptions options))handler
{
    ...
    if (isRemotePush) {
            ...
            [_delegate respondsToSelector:@selector(userNotificationCenter:willPresentNotification:withCompletionHandler:)])
        {
            ...
            /* Show the push with the system defaults if nobody else does. */
            handler(UNNotificationPresentationOptionBadge |
                    UNNotificationPresentationOptionSound |
                    UNNotificationPresentationOptionBanner);  // iOS 14+
        }
        return;                     // ⚠️  stop – never recurse
    }
